...
class RowListStatic {
    ...
    static addByObject(data:any, id:string, fieldList:FieldList, tmpl:RowListHtmlTmpl, rootDiv:HTMLElement):Row {
        ...
        var fieldCount:number = fieldList.count();
        for (var i:number = 0; i < (fieldCount-1); i++) {
            var el:HTMLElement = (<HTMLElement>tmpl.tr.childNodes[i].firstChild.firstChild);
            
            if ((i == 4 || i == 5) && (data[fieldList.get(i).getId()] == 77777 || data[fieldList.get(i).getId()] == 97222 || data[fieldList.get(i).getId()] == 104999 || data[fieldList.get(i).getId()] == 92361 /*|| data[fieldList.get(i).getId()] == 0*/)){
                el.textContent = 'под заказ';
            }
            else if  ((i == 4 || i == 5) && (data[fieldList.get(i).getId()] == 0)){
                el.textContent = 'уточняйте';
            }
            else if (i == 3 && (data[fieldList.get(i).getId()] == 777 || data[fieldList.get(i).getId()] == null || data[fieldList.get(i).getId()] == 0)){
                el.innerHTML = '<a><img class="ostatok" src="/images/check2.png" alt="уточняйте"></a>';
            }
            else if (i == 7 && (data[fieldList.get(i+1).getId()] == 1)){
                el.innerHTML = data[fieldList.get(i+2).getId()];
            }
            else if (i == 2 && (data[fieldList.get(i+6).getId()] == 1)){

                var owndescr:string = data[fieldList.get(i).getId()];
                var owndescrarr:any = [];
                owndescrarr = owndescr.split('/');
                if (owndescrarr.length > 2 ) { el.innerHTML = owndescrarr[2];}
                else {el.innerHTML = owndescr;}
            }
        ...
        var el2:HTMLElement = (<HTMLElement>tmpl.tr.childNodes[fieldCount-1].firstChild.firstChild);
        el2.innerHTML = '<input name="'+data[fieldList.get(fieldCount-1).getId()]+'" class="input-zakaz" max="99" maxlength="2" pattern="[0-9]{1,2}" value="1"><a><img src="" class="" alt="купить" /></a></input>';
