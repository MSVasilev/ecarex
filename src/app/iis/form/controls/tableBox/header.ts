...
class Header extends Control {
    ...
    createHtmlElement():HTMLElement {
        ...
        for (var i:number = 0; i < fieldCount; i++) {
            if (i == 4) {
                htmlEl += '<div class="' + FieldListStatic.getFieldCSSClassName(i, this.tableBox) + '" id="defaultSort"><div class="fa">' + this.tableBox.fieldList.get(i).description + '</div></div>';
            }
            else {
    ...
    public setCurrentField(event:MouseEvent) {
        event.stopPropagation();
